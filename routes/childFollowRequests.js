  ...
  return res.json({
    ...
    message:
      request.status === "APPROVED"
        ? "این درخواست قبلاً توسط یکی از والدین پذیرفته شده است."
        : request.status === "REJECTED_BY_PARENT"
        ? "این درخواست قبلاً توسط یکی از والدین رد شده است."
      ...
      const deciderLabel =
  isParent.role === "father"
    ? "پدر"
    : isParent.role === "mother"
    ? "مادر"
    : "یکی از والدین";

const updateOtherParentNotifications = async (decisionText) => {
  await prisma.notification.updateMany({
    ...
    data: {
      read: true,
      title: "درخواست فالو بررسی شد",
      body: `این درخواست قبلاً توسط ${deciderLabel} ${decisionText}.`,
    },
  });
};
      ...
      if (action === "REJECT") {
        ...
        await updateOtherParentNotifications("رد شده است");
      ...
      if (action === "CHANGE_ROLE") {
        ...
        await updateOtherParentNotifications("با تغییر نقش بررسی شده است");
        return res.json({ ok: true });
      }
      ...
      await updateOtherParentNotifications("پذیرفته شده است");
      return res.json({ ok: true });
    } catch (error) {
